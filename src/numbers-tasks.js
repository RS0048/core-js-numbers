...
function roundToPowerOfTen(num, pow) {
  const roundingDegree = 10 ** pow;
  return Math.round(num / roundingDegree) * roundingDegree;
}
...
function getSumOfDigits(num) {
  const numMassive = String(num).split('');
  let sum = 0;
  for (let i = 0; i < numMassive.length; i += 1) {
    sum += Number(numMassive[i]);
  }
  return sum;
}
